...
def kl_divergence(
    ...
    for col in solution.columns:
        ...
        # Clip both the min and max following Kaggle conventions for
        # related metrics like log loss
        # Clipping the max avoids cases where the loss would be infinite
        # or undefined, clipping the min prevents users from playing games
...
def score(
    ...
    The KL divergence is technically undefined/infinite where the
    target equals zero.

    This implementation always assigns those cases a score of zero;
    effectively removing them from consideration. The predictions in
    each row must add to one so any probability assigned to a case where
    y == 0 reduces another prediction where y > 0, so crucially there
    ...
    solution: pd.DataFrame
    submission: pd.DataFrame
    epsilon: KL divergence is undefined for p=0 or p=1. If epsilon is not
    null, solution and submission probabilities are clipped to max(eps, min(1 - eps, p).
    row_id_column_name: str
